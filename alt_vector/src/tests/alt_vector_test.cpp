...
SCENARIO("compact vector assign with same value")
{
    ...
    CHECK_THROWS_AS(vector.assign(5U, 1), ContainerOverflow);
}
...
SCENARIO("compact vector assign with iterators")
{
    ...
    CHECK_THROWS_AS(vector.assign(too_big.begin(), too_big.end()), ContainerOverflow);

    const auto const_too_big = too_big;
    CHECK_THROWS_AS(vector.assign(const_too_big.begin(), const_too_big.end()), ContainerOverflow);
}
...
SCENARIO("compact vector assign with initializer list")
{
    ...
    CHECK_THROWS_AS(vector.assign({0, 0, 0, 0, 0}), ContainerOverflow);
}
...
SCENARIO("compact vector at operators")
{
    ...
    CHECK(vector.at(0U) == 1);
    CHECK(vector.at(1U) == 0);
    CHECK_THROWS_AS(vector.at(5U), ContainerOverflow);

    CHECK(const_vector.at(0U) == 1);
    CHECK(const_vector.at(1U) == 0);
    CHECK_THROWS_AS(const_vector.at(5U), ContainerOverflow);
}
...
SCENARIO("compact vector capacity modifiers")
{
    ...
    // nothing happens
    vector.reserve(4U);
    CHECK_THROWS_AS(vector.reserve(5U), ContainerOverflow);
}
...
SCENARIO("compact vector push_back/emplace_back")
{
    ...
    std::int32_t five = 1;
    CHECK_THROWS_AS(vector.push_back(five), ContainerOverflow);
    CHECK_THROWS_AS(vector.push_back(6), ContainerOverflow);
    CHECK_THROWS_AS(vector.emplace_back(7), ContainerOverflow);
}
...
SCENARIO("compact vector resize")
{
    ...
    CHECK_THROWS_AS(vector.resize(5U), ContainerOverflow);
    ...
    CHECK_THROWS_AS(vector.resize(5U, 1), ContainerOverflow);
}
...
SCENARIO("compact vector erase method")
{
    GIVEN("A AltVector with elements {1, 0, 1, 0}")
    {
        ...
        WHEN("erasing an element with an invalid iterator")
        {
            ...
            {
                CHECK_THROWS_AS(vector.erase(vector.end()), BaseException);
                CHECK_THROWS_AS(vector.erase(vector.begin() - 1), BaseException);
            }
        }
    }
}
